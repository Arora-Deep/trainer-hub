...
import { BookOpen, Play, CheckCircle, Search, Star, Clock, Timer, ChevronRight, Sparkles, Radio } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Live, self-paced and hybrid courses you're enrolled in</p>
        </div>
        ...
        {filtered.map((c) => {
          ...
          return (
            <motion.div key={c.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              <Card className={`transition-all hover:shadow-md ${isCompleted ? "border-success/20" : ""}`}>
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between gap-4">
                    <Link to={`/student/courses/${c.id}`} className="flex items-start gap-3 flex-1 min-w-0 group">
